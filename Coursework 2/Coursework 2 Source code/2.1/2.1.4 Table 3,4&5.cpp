...
int main()
{
	...
	cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
	cout << "Press any key to obtain the first table..." << endl;
	_getch();

	ofstream output;

	output.open("P:/grid1.csv");
	if (!output.is_open())
	{
	...
	double valueOld = 0.;
	double diff = 0; double diffOld = 0; double diff_abs; 
	double rate;

	iMax = 10000;
	cout << "Now ouput iMax, jMax, option value and convergence rate...\n" << endl;
	for (int n = 10; n <= 10000; n *= 2)
	{
		...
		cout << iMax << " ," << jMax << " , " << value << " , " << diff_abs << " , " << log(rate) / log(2) << endl;
		output << iMax << " ," << jMax << " , " << value << ", " << diff_abs << "," << log(rate) / log(2) << endl;
		valueOld = value;
		diffOld = diff;
	}
	...
	cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
	cout << "Press any key to obtain the second table..." << endl;
	_getch();

	output.open("P:/grid2.csv");
	if (!output.is_open())
	{
		cout << "File not opened \n";
		throw; // stop the program here
	...
	diffOld = 0;
	jMax = 10000;
	cout << "Now output the similar results...\n" << endl;
	...
	cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
	cout << "Press any key to obtain the last table..." << endl;
	_getch();



	output.open("P:/grid3.csv");
	if (!output.is_open())
	{
		cout << "File not opened \n";
		throw; // stop the program here
	...
	diff = 0;
	diffOld = 0;
	cout << "Now output n, value extrapolation and convergence rate...\n" << endl;
	...
	for (int n = 10; n <= 10000; n *= 2)
	{
		...
		diff = value - valueOld;
		diff_abs = abs(diff);
		double value_ep = (4.*value - valueOld) / 3.;
		rate = abs(diffOld / diff);
		cout << iMax << " ," << jMax << " , " << value << " , " << diff_abs << " , " << log(rate) / log(2) << endl;
		output << iMax << " ," << jMax << " , " << value << ", " << diff_abs << "," << value_ep << "," << log(rate) / log(2) << endl;
		valueOld = value;
		diffOld = diff;
	}
