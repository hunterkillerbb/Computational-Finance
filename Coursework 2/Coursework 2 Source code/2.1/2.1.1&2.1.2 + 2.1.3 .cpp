...

{
...

	{
		...
		d[jMax] = -0.5 * 1 / dS * vOld[jMax] + 0.5 * 1 / dS * vOld[jMax - 1] + R*exp(-(kappa + r)*(T - (i + 0.5)*dt));
		          //S[jMax] * R * exp(-(kappa + r)*(T - i *dt)) + C / (alpha + r) * (exp(-alpha * t) - exp(-alpha + r*(i*dt - T))) + X*R * (exp(r*(T - i*dt)) - exp(-(kappa + r)*(T - i*dt)));


		// solve with thomas solver and interpolation
		vNew = d;
...
int main()
{
	...
	cout << "The option value V as S0 = 44.12 is: " << ConvertibleBond_CN(C, alpha, beta, mu, kappa, R, S0, r, X, sigma, T, F, iMax, jMax, SMax, 1, 1.e-8, 10000) << endl;




	cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
	cout << "Press any key to generate data in two cases..." << endl;
	_getch();

	ofstream output, output1;

	output.open("P:/Cases.csv");
	...
	{
		output << "case1" << "," << S0 << "," << ConvertibleBond_CN(C, alpha, beta, mu, kappa, R, S0, r, X, sigma, T, F, iMax, jMax, SMax, 1, 1.e-8, 10000) << endl;

	}

	beta = 0.198;  sigma = 4.98;
	for (S0 = 0; S0 <= 105; S0 += 5)
	{
		output << "case2" << "," << S0 << "," << ConvertibleBond_CN(C, alpha, beta, mu, kappa, R, S0, r, X, sigma, T, F, iMax, jMax, SMax, 1, 1.e-8, 10000) << endl;
	}
	...
	cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
	cout << "Press any key to continue with the second sheet..." << endl;
	_getch();

	output1.open("P:/Beta and sigma.csv");
	if (!output1.is_open())
	{
		cout << "File not opened \n";
		throw; // stop the program here
	...
	beta = 0.198; sigma = 4.98;
		...
		for (beta = -0.5; beta <= 2; beta += 0.5)
		{
			for (sigma = 0; sigma <= 10; sigma += 0.5)
			{
				cout << beta << endl;
				cout << sigma << endl;
				output1 << beta << "," << sigma << "," << ConvertibleBond_CN(C, alpha, beta, mu, kappa, R, S0, r, X, sigma, T, F, iMax, jMax, SMax, 1, 1.e-8, 10000) << endl;
			}
		}
